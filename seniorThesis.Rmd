...
What I am looking at is Carnegie Mellon's room assignment (Room Selection) process and how its mechanism fits with the literature as well as examining preferences and strategies of students. One of the main extracurriculars I’ve been involved in during my time at Carnegie Mellon has been with Housing and residence life and so digging deeper into the mechanics of the Room Selection process through an economics lenses is a perfect marriage of my academic and extracurricular interests.  
...
ggplot(classParticipationCount,aes(x = variable, y = classParticipationRS)) +
                  geom_bar(aes(fill = Class_ordered), stat = "identity", 
                           position = "dodge") +
                  labs(title = "Figure 1: Room Selection Participation by class for 2016 and 2017",
                        x = "Year", y = "Count",
                       fill = "Class")
...
ggplot(totalClassParticipationCount,aes(x = variable, y = classParticipationRS)) +
                  geom_bar(aes(fill = Class_ordered1), stat = "identity", 
                           position = "fill") +
                  labs(title = "Figure 2: Room Selection Participation by class by proportion \nto total participants for 2016 and 2017",
                       x = "Year", y = "Proportion",
                       fill = "Class")
```
...
surveyResults.copy %>% 
        ...
        ggplot(aes(x = blockAsPrioritySelection)) + geom_bar() +
                   labs(title = "Figure 3: Perception of Block Housing as Priority Selection",
...
ggplot(data = surveyResults.copy, aes(x = surveyResults.copy$blockSatisfaction),
                                      na.rm = TRUE) + 
  geom_bar() +
  labs(title = "Figure 4: Distribution of Satisfaction with Block Assignments",
...
In addition to getting capacity and participation data, I was also interested in getting any opinion/survey data from students regarding their perception of the process as added insights to the research since perception also influences behavior (i.e. join or leave Room Selection, which phase to participate in). It was harder to get the data as there are confidentiality (i.e. personal identifiable information) considerations on Housing's part, but I was able to get data from the 2018 Room Selection exit survey, a survey that Housing gives to residents who participate in Room Selection. Take-up into the survey is voluntary. As such, it's a sample of 330 responses.[^2] I recognize that because the final working sample of about 300 responses is just about 10% of those eligible for Room Selection (or about 15% of those who actually participated) it may not be the most representative of the perceptions of the entire population, particularly of those who went off campus whose data aren't included in the survey. Additionally I recognize that with survey data there may usually be a skew toward the positive and/or negative extreme (positive, negative in terms of perception). In this case, it looks like there is a skew towards the positive end, however one would need to conduct additional tests or research to examine the perceptions more closely. As such, I use the results more to get a reference and broad overview of students' *general* perceptions about the process. For instance, one can note the general class and phase compositions of Room Selection participants, as seen in Figures 7-10 (in Appendix), with first-years dominating Block Housing and General Selection significantly by juniors and those in Retention phase. And in Figures 12-17, one can see the general perceptions of fairness, simplicity, and stressfulness. It seems that the process is generally perceived to be fair and simple in aggregate, but broken down by class more senior students perceiving it slightly fairer than the more junior ones (Figure 12), a point that can be explained to be through the lenses of individual rationality and the retention phase. 

[^2]: Of which there were about 30 N/A, empty, or RA/CA responses that I had to clean out resulting in a working sample of about 300 responses.
...
Normative assignments based on these specifications: [^4]    
...
[^10]: In reality one can see that may not always be the case, and as I have shown above if we include being 'unassigned' into our welfare calculations by assigning it to 0 welfare (as opposed to omitting it) to try to loosen the assumption, it changes the welfare and fairness numbers. Again, of course being 'unassigned' doesn't always have to be the last choice but rather in the middle. In that case it would act like a kind of 'preference floor' such that anything worse (or the perception that one might get something worse) would result in the participant to prefer to leave the process.

\newpage

## Conclusion

Through this paper I gave a description of the Room Selection process at Carnegie Mellon and closer look at some aspects of the process. From previous literature we identified existing housing mechanisms, such as Shapley and Scarf's Top Trading Cycles, (Budish, Che, Kojima and Milgrom, 2013) (Hylland and Zeckhauser, 1979) (Bogomolnaia and Moulin, 2001)'s probabilistic serial mechanisms, or Abdulkadiroglu and Sonmez's *You Request My House-I Get Your Turn* (YRMH-IGYT) algorithm, of which CMU's Room Selection process is a similar variant of the random serial dictatorship from the YRMH-IGYT. From data obtained from Housing Services over past couple years iterations of Room Selection, we were able to see the breakdown in participation by phases as well as some sense of breakdowns by class year. Additionally, through the Spliddit mechanism (Goldman & Procaccia, 2014) I was able to model iterations of the Room Selection model that are based proportionally on numbers from data. Through the model I was able to test the significance of the retention phase by taking it out and holding necessary assumptions about the preferences of those affected. We see that the retention phase is a guarantor of individual rationality for existing tenants and therefore improves general welfare [^11] for the entire process. However, by getting rid of retention we improve fairness (or, perceived thereof). Specifically, the case where everyone in the model ranks a particular house that was held by an existing tenant (and would have been retained), inequality was slightly higher than the other cases, signifying the role of 'legacy houses' in reducing fairness. It is an important point to note in that improvement in fairness may lead to increased trust and confidence in the process and thereby increase participation in the process.  
...
Special thanks to Faculty Advisor Professor Onur Kesten in Economics for the support throughout the year, Associate Director of Housing Services Lisa Hartman and the rest of Housing Services for graciously providing the Room Selection data, and Carnegie Mellon student Aijin Wang for helping with R.
...
sum2 <- read.csv("~/Desktop/roomselection/summary2.csv")
...
#### List of all buildings and capacities available for Room Selection
##### Table 2
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# buildings for RS (capacity for RS)
sum3 <- read.csv("~/Desktop/roomselection/summary3.csv")
colnames(sum3)[2] <- "Capacity"
sum3 <- sum3[-c(19),]

sum3 %>%
  kable() %>%
  kable_styling(full_width = F) %>%
  footnote(number = c("2018: Includes 62 student staff (RA/CAs) in those buildings", 
                      "2018: Total available capacity of 1963"))
```
...
sum1 <- read.csv("~/Desktop/roomselection/summary1.csv")
...
##### Table 4: Participation by phase
```{r, echo=FALSE, warning=FALSE, message=FALSE}
sum4_t <- read.csv("~/Desktop/roomselection/summary4_t.csv")
...
```
##### Table 5: (Cumulative) occupancies
```{r, echo=FALSE, warning=FALSE, message=FALSE}
sum5_t <- read.csv("~/Desktop/roomselection/summary5_t.csv")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
sum4 <- read.csv("~/Desktop/roomselection/summary4.csv")
sum5 <- read.csv("~/Desktop/roomselection/summary5.csv")

colnames(sum4) <- c("Participation", "2014", "2015", "2016", "2017", "2018")
colnames(sum5) <- c("Occupancy (cumulative)", "2014", "2015", "2016", "2017", "2018")

newSum4 <- reshape2::melt(sum4, value.name = "RSparticip", 
                             varnames = c("Phase", "participation"))
newSum5 <- reshape2::melt(sum5, value.name = "Occupancy", 
                             varnames = c("Phase", "occupancy"))

newSum4_phases <- newSum4[-c(5,10,15,20,25),]
newSum4_totals <- newSum4[c(5,10,15,20,25),]

Phase_ordered <- ordered(newSum4_phases$Participation,levels = c("Retention", "Block Housing", "General Selection", "Open Assignments"))
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
ggplot(newSum4_phases,aes(x = variable, y = RSparticip)) +
                  geom_bar(aes(fill = Phase_ordered), stat = "identity",
                           position = "dodge") +
                  labs(title = "Figure 5: Room Selection Participation by Phase between 2014 and 2018",
                        x = "Year", y = "Count",
                       fill = "Phase")

ggplot(newSum4_totals,aes(x = variable, y = RSparticip)) +
                  geom_bar(stat = "identity") +
                  labs(title = "Figure 6: Room Selection Participation between 2014 and 2018",
                        x = "Year", y = "Count",
                       fill = "Phase")
```
...
### Appendix: 2018 Room Selection Exit Survey Data

Note: Sample size for each of the graphs is 300 (aforementioned), those who had responded to the survey  
...
ggplot(data = surveyResults.copy, aes(x = Year_ordered, 
                                      na.rm = TRUE)) + 
      geom_bar() +
      labs(x = "Class", y = "Count", title = "Figure 7: Distribution of Class")
...
ggplot(data = surveyResults.copy, aes(x = Phase_ordered)) + 
  ...
  labs(title = "Distribution of Participation in Room Selection Phases",
       ...
       y = "Count", title = "Figure 8: Distribution of Phases")
...
ggplot(data = surveyResults.copy, aes(x = Phase_ordered,
      ...
      labs(x = "Phase of Room Selection", y = "Count", fill = "Class",
           title = "Figure 10: Phase by Class")
...
ggplot(data = surveyResults.copy, aes(x = surveyResults.copy$Fairness,
      ...
      labs(x = "Fairness (1=least; 5=most)", y = "Count",
           title = "Figure 11: Distribution of Fairness")
...
ggplot(data = surveyResults.copy, aes(x = surveyResults.copy$Fairness,
      ...
      labs(x = "Fairness (1=least; 5=most)", y = "Count", fill = "Class",
           title = "Figure 12: Perception of fairness by Class")
...
ggplot(data = surveyResults.copy, aes(x = surveyResults.copy$Simplicity,
      ...
      labs(x = "Simplicity (1=least; 5=most)", y = "Count", fill = "Class",
           title = "Figure 13: Perception of simplicity by Class")
...
ggplot(data = surveyResults.copy, aes(x = surveyResults.copy$Stressfulness,
      ...
      labs(x = "Stressfulness (1=least; 5=most)", y = "Count", fill = "Class",
           title = "Figure 14: Perception of stressfulness by Class")
...
ggplot(data = surveyResults.copy, aes(x = surveyResults.copy$Fairness,
      ...
      labs(x = "Fairness (1=least; 5=most)", y = "Count", 
           ...
           title = "Figure 15: Perception of fairness by Phase")
...
ggplot(data = surveyResults.copy, aes(x = surveyResults.copy$Simplicity,
      ...
      labs(x = "Simplicity (1=least; 5=most)", y = "Count", 
           fill = "Phase of Room Selection",
           title = "Figure 16: Perception of simplicity by Phase")
...
ggplot(data = surveyResults.copy, aes(x = surveyResults.copy$Stressfulness,
      ...
      geom_bar() +
      labs(x = "Stressfulness (1=least; 5=most)", y = "Count", fill = "Class",
           title = "Figure 17: Perception of stressfulness by Phase")
...
ggplot(data = surveyResults.copy, aes(x = surveyResults.copy$`Satisfaction*`,
      ...
      labs(caption = "Satisfaction a general term to indicate a participant's opinion, 
           ...
           x = "Satisfaction (1=least; 5=most)", 
           y = "Count",
           title = "Figure 18: Distribution of satisfaction with final assignments")
```
\newpage

### Appendix: Spliddit spreadsheet results
